...
import '../assets/CentroidPolygon.css';
...
const CentroidPolygon: React.FC = () => {
  const [kmlFile, setKmlFile] = useState<File | null>(null);
  const [placemarkName, setPlacemarkName] = useState<string>('Rumah {n}');
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/polygon-centroid', {
  	method: 'POST',
  	body: formData
	});
      ...
      let filename = 'centroid_result.kml';
      ...
      alert("Titik Centroid berhasil diekstrak dan diunduh!");
    } catch (error: any) {
  ...
  return (
    <MainLayout pageTitle="Centroid Polygon" activeMenu="centroid-polygon">
      <div className="card">
          ...
          <form onSubmit={handleGenerate}>
            ...
            <div className="form-group">
              ...
              <div className="cp-icon-grid">
                {AVAILABLE_ICONS.map((icon) => (
                  <div 
                    key={icon.name}
                    className={`cp-icon-option ${selectedIcon === icon.name ? 'selected' : ''}`}
                    onClick={() => setSelectedIcon(icon.name)}
                  >
                    <img src={icon.url} alt={icon.name} className="cp-icon-image" />
                    <span className="cp-icon-name">{icon.name}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="button-separator">
              <button type="submit" className="btn-block" disabled={isProcessing}>
                {isProcessing ? 'Memproses...' : 'Proses Centroid'}
              </button>
            </div>
...
export default CentroidPolygon;